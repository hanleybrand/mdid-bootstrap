...
class ArtstorSearch(FederatedSearch):
    ...
    def hits_count(self, keyword):
        ...
        try:
            ...
            log.debug('rooibos.federatedsearch.artstor:URLError = %s' % e)
            return 0
        soup = BeautifulSoup(response)
        try:
            return int(soup.find('numberofrecords').contents[0])
        except Exception as e:
            log.debug('rooibos.federatedsearch.artstor:Exception = %s' % e)
            return 0
    ...
    def search(self, keyword, page=1, pagesize=50):
        ...
        try:
            ...
            log.debug('rooibos.federatedsearch.artstor:ExpatError = %s' % e)
            total = 0
        if not total:
            return None
